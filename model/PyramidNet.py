...
#             padding = torch.autograd.Variable(torch.cuda.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1]).fill_(0))
            padding = torch.zeros(batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1], device=shortcut.device)
...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=False):
        ...
        if self.dataset == 'cifar10' or self.dataset == 'cifar100' or self.dataset == 'food101':
            self.inplanes = 16
            if bottleneck == True:
                n = int((depth - 2) / 9)
                block = Bottleneck
